...
int main(int argc, char **argv)
{
    ...
    int is_late = 1;
    double barrier_time, tlocal = 0.0, tmax = 0.0;
    ...
    if (t > barrier_time) {
        ...
        // Do something
        double tt = MPI_Wtime();
        printf("process %3d: local time %.6f, global time %.6f (offset %.12f)\n",
               rank, tt, tt + clock_offset, clock_offset < 0 ? -clock_offset : clock_offset);
    }

    // TODO: реализовать экспоненциальное увеличение delta при запаздывании процессов
